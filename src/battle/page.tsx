...
export default function BattlePage() {
    ...
    useEffect(() => {
        ...
        try {
            const decksKey = `${activeProfile}-decks`;
            const cardBackKey = `${activeProfile}-cardBackImage`;

            const decksFromStorage: Deck[] = JSON.parse(localStorage.getItem(decksKey) || '[]');
            setSavedDecks(decksFromStorage);
            
            const savedCardBack = localStorage.getItem(cardBackKey);
            if (savedCardBack) {
