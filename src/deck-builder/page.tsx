...
export default function DeckBuilderPage() {
  ...
  const getCollectionKey = useCallback(() => `${activeProfile}-cardCollection`, [activeProfile]);
  const getDecksKey = useCallback(() => `${activeProfile}-decks`, [activeProfile]);

  useEffect(() => {
    setIsClient(true);
    if (!activeProfile) return;
    try {
      const savedCollection = JSON.parse(localStorage.getItem(getCollectionKey()) || '[]');
      const savedDecks = JSON.parse(localStorage.getItem(getDecksKey()) || '[]');
      setCollection(savedCollection);

      if (savedDecks.length > 0) {
        setDecks(savedDecks);
        setActiveDeckId(d => savedDecks.find((dk: Deck) => dk.id === d) ? d : savedDecks[0].id);
      } else {
  ...
  }, [activeProfile, getCollectionKey, getDecksKey]);
  ...
  const updateDecks = (newDecks: Deck[]) => {
    setDecks(newDecks);
    if(activeProfile) {
        localStorage.setItem(getDecksKey(), JSON.stringify(newDecks));
    }
  }
