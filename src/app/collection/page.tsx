...
export default function CollectionPage() {
  ...
  const handleDeleteCard = (cardId?: string) => {
    if (!cardId) return;
    const newCollection = cardCollection.filter(card => card.id !== cardId);
  ...
  const handleSaveEdit = (originalCard: CardData, updatedCard: CardData) => {
    ...
    const newCollection = cardCollection.map(card => card.id === updatedCard.id ? updatedCard : card);
    updateCollection(newCollection);
  ...
  const handleEvolveCard = (cardToEvolve: CardData) => {
    ...
    if (removeItem(material, requiredAmount)) {
        ...
        const cardIndex = cardCollection.findIndex(c => c.id === cardToEvolve.id);
